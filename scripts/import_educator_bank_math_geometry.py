#!/usr/bin/env python3
"""Import Geometry and Trigonometry questions into SAT Educator Bank 1.

Source: SAT Bank 1/Math/Geometry and Trignometry 1.pdf
...
Summer Geometry and Trigonometry stems.
"""
...
UNANSWERED_NAME = "Educator-Bank-1-Geometry-Trigonometry-Unanswered.pdf"
...
def build_item(
    ...
    if domain != DOMAIN:
        ...
        if not skill:
            ...
            skill = clean_text(m.group(1)) if m else "Geometry and Trigonometry"
        if not difficulty:
...
def main() -> None:
    ...
    print("Building unanswered Geometry and Trigonometry PDF...")
    make_unanswered_pdf(BANK_PDF, unanswered_path)
    answered_copy = PUBLIC_MATH / "Educator-Bank-1-Geometry-Trigonometry.pdf"
    ...
    # Re-import replaces prior Educator Bank Geometry and Trigonometry rows from this script.
    kept = [
        q
        for q in existing
        if not (
    ...
    for n, g in enumerate(groups, 1):
        ...
        if fp and fp in summer_fps and len(fp) >= 80:
            print(f"skip {qid}: prompt fingerprint matches Summer Geometry and Trigonometry")
            skipped_dup += 1
            continue
        ...
        if n % 10 == 0 or n == 1:
            ...
            print(f"geo {n}/{len(groups)} {qid}: {preview}")
    ...
    print(f"Added {len(added)} Educator Bank Geometry and Trigonometry questions "
    ...
    if overlap:
        ...
        raise SystemExit(1)
    print(f"Summer Geometry and Trigonometry overlap check: OK (0 / {len(summer_ids)})")
