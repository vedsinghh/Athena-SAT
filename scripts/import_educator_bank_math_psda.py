#!/usr/bin/env python3
"""Import Problem-Solving and Data Analysis questions into SAT Educator Bank 1.

Source: SAT Bank 1/Math/Problem-Solving and Data Analysis 1.pdf
...
Summer Problem-Solving and Data Analysis stems.
"""
...
UNANSWERED_NAME = "Educator-Bank-1-Problem-Solving-Data-Analysis-Unanswered.pdf"
...
# Hand-fixes filled in after PDF visual QA when OCR leaves hollow stems.
...
def build_item(
    ...
    if domain != DOMAIN:
        ...
        if not skill:
            ...
            skill = clean_text(m.group(1)) if m else "Problem-Solving and Data Analysis"
        if not difficulty:
...
def main() -> None:
    ...
    print("Building unanswered Problem-Solving and Data Analysis PDF...")
    make_unanswered_pdf(BANK_PDF, unanswered_path)
    answered_copy = PUBLIC_MATH / "Educator-Bank-1-Problem-Solving-Data-Analysis.pdf"
    ...
    # Re-import replaces prior Educator Bank Problem-Solving and Data Analysis rows from this script.
    kept = [
        q
        for q in existing
        if not (
    ...
    for n, g in enumerate(groups, 1):
        ...
        domain, _skill, _diff = parse_meta(g["text"], "Math", [DOMAIN, "Algebra", "Geometry and Trigonometry", "Advanced Math"])
        if domain and domain != DOMAIN:
            skipped_other += 1
            continue
        ...
        if fp and fp in summer_fps and len(fp) >= 80:
            print(f"skip {qid}: prompt fingerprint matches Summer Problem-Solving and Data Analysis")
            skipped_dup += 1
            continue
        ...
        if n % 10 == 0 or n == 1:
            ...
            print(f"psda {n}/{len(groups)} {qid}: {preview}")
    ...
    print(f"Added {len(added)} Educator Bank Problem-Solving and Data Analysis questions "
    ...
    if overlap:
        ...
        raise SystemExit(1)
    print(f"Summer Problem-Solving and Data Analysis overlap check: OK (0 / {len(summer_ids)})")
